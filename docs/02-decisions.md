...
- **Choice:** Search Box and ask Box AI before composition. On an article, use that file as a hint when the question refers to the page or normal search finds nothing. Readers can pin the article for a page-only question or remove it to search the whole library. One Gateway request writes the streamed answer and cites `/products/...` URLs.
